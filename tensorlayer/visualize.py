...
def make_save_images(images, size, image_path):
    """Save mutiple images into one single image.
    ...
    rot = cv2.warpAffine(copy,M,(cols,rows),borderMode=cv2.BORDER_CONSTANT, borderValue=(255, 255, 255))
    flp = cv2.flip(rot,1)
    
    save_img = np.ones((cols, cols), np.float32)*255
    save_img[0:rows, 0:cols] = copy
    save_img[rows:cols-1, 1:cols] = flp[1:, :cols-1]
        
    #images= np.concatenate((images, flp), axis=0)
    return imsave(save_img, size, image_path)
